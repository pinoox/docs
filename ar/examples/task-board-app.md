# شرح تطبيقي: لوحة المهام (Todo)

[← العودة إلى الفهرس](../README.md)

**لوحة مهام يومية**: أضف مهامًا، علّمها كمكتملة، و**صفِّها** بسلسلة استعلام (`?status=pending`). مثال عملي للوحات الداخلية وتعلّم التصفية بالاستعلام في المتحكمات.

**الحزمة (Package):** `com_acme_tasks`  
**الرابط (URL):** `http://localhost/pinoox/tasks`  
**الكود المصدري الكامل:** [docs/source/task-board-app/](../../source/task-board-app/) — انسخه إلى `apps/`
---

## المتطلبات المسبقة

- [الطلبات (Requests)](../basic/requests.md) — معاملات الاستعلام
- [قاعدة البيانات / Eloquent](../database/getting-started.md)
...
> **سلسلة الاستعلام:** `$request->get('status')` يقرأ `?status=pending` — دون حاجة لمسار إضافي.

---

## الخطوة 6 — قالب Twig (مع CSS مضمّن)
...
1. أضف عدة مهام.
2. علّم بعضها كمكتملة بـ ✓.
3. التصفية «Open» تعرض `pending` فقط.
4. استخدم ↩ لإعادة فتح المهمة.
...
| الترقية | التوثيق |
|---------|-----|
| الأولوية / تاريخ الاستحقاق | [Mutators & casts](../eloquent-orm/mutators-casts.md) |
| واجهة API للجوال | [شرح Notes API](./simple-api-app.md) |
| تسجيل دخول المستخدم | [إدارة المستخدمين](../advanced/user-management.md) |
...
- [نطاقات Eloquent](../eloquent-orm/getting-started.md)
