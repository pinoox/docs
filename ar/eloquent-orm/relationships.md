# علاقات Eloquent (Relationships)

[← العودة إلى الفهرس](../README.md)

علاقات Eloquent في Pinoox 3.x تستخدم **`hasMany`** و**`belongsTo`** وواجهات API ذات الصلة القياسية. المثال أدناه مدونة بسيطة داخل تطبيق Pinoox:

---

## النماذج

```php
<?php
namespace App\com_acme_blog\Model;

use Pinoox\Component\Database\Model;
use Pinoox\Model\UserModel;

class PostModel extends Model
{
    protected $table = 'posts';
    protected $primaryKey = 'post_id';

    protected $fillable = ['user_id', 'title', 'body', 'status'];

    public function author()
    {
        return $this->belongsTo(UserModel::class, 'user_id', 'user_id');
    }

    public function comments()
    {
        return $this->hasMany(CommentModel::class, 'post_id', 'post_id');
    }
}
```

```php
<?php
namespace App\com_acme_blog\Model;

class CommentModel extends Model
{
    protected $table = 'comments';
    protected $primaryKey = 'comment_id';

    protected $fillable = ['post_id', 'user_id', 'body'];

    public function post()
    {
        return $this->belongsTo(PostModel::class, 'post_id', 'post_id');
    }

    public function author()
    {
        return $this->belongsTo(UserModel::class, 'user_id', 'user_id');
    }
}
```

---

## Eager loading

```php
$posts = PostModel::with(['author', 'comments'])->get();

foreach ($posts as $post) {
    echo $post->author->email;
    echo $post->comments->count();
}
```

---

## Lazy loading

```php
$post = PostModel::find(1);
$comments = $post->comments;   // separate query
$author = $post->author;
```

---

## الإنشاء عبر علاقة

```php
$post = PostModel::find(1);

$post->comments()->create([
    'user_id' => Auth::id(),
    'body' => 'New comment',
]);
```

---

## whereHas

```php
$posts = PostModel::whereHas('comments', function ($q) {
    $q->where('created_at', '>=', now()->subDay());
})->get();
```

---

## belongsToMany (اختياري)

```php
public function tags()
{
    return $this->belongsToMany(
        TagModel::class,
        'post_tags',
        'post_id',
        'tag_id',
    );
}
```

جدول pivot `post_tags` يتلقى أيضًا بادئة التطبيق.

---

## نصائح

- FK إلى `UserModel` النواة: `user_id` → `pincore_user.user_id`.
- استخدم دائمًا `with()` لقوائم API لتجنب N+1.
- استخدم أسماء علاقات Eloquent المفرد/الجمع القياسية.

---

## وثائق ذات صلة

- [البدء مع Eloquent](./getting-started.md)
- [المجموعات (Collections)](./collections.md)
- [التسلسل (Serialization)](./serialization.md)

---

[← العودة إلى الفهرس](../README.md)
