...
`php pinoox fe dev` resolves URLs from the app router. You do not need a filled `.env` for local dev — missing `VITE_*` keys are injected at runtime.

Optional manual overrides in `theme/default/.env`:

```env
# Persist autogenerated VITE_* block on each fe dev run (default: false)
ENV_SERVER_SYNC=false

# Manual overrides (used when set):
