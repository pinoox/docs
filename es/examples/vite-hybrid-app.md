# Tutorial: híbrido Vite (Twig + widget JS)

[← Volver al índice](../README.md)

Una **página de producto** con HTML completo en Twig (SEO y meta en PHP) más un **widget Vite** interactivo (Vanilla JS) que calcula el precio por cantidad. Perfil **hybrid** — sin Vue/React.

**Package:** `com_acme_vite_shop`  
**URL:** `http://localhost/pinoox/shop`  
**Profile:** `hybrid` · **stack:** `vite`  
**Código fuente completo:** [docs/source/vite-hybrid-app/](../../source/vite-hybrid-app/) — copiar a `apps/`
---

## Requisitos previos

- Node.js 18+
- [Views](../basic/views.md) y [`vite_tags()`](../basic/templates.md)
...
## Paso 3 — Vite en el tema (sin Vue/React)
...
## Paso 4 — Widget Vanilla JS

**`src/boot.js`** (compartido con stubs SPA):
...
> **Regla híbrida:** título, descripción y contenido principal viven en Twig — el widget solo monta el bloque interactivo. Los datos iniciales vienen de `bootstrap`, no de una petición API duplicada.

---

## Paso 6 — `.env` y desarrollo
...
Abre la página en MAMP; al cambiar la cantidad se actualiza el total en vivo.
...
Manifest en `dist/.vite/manifest.json` — Twig inyecta la etiqueta `<script>` correcta en producción.

---

## Múltiples entradas (opcional)

Extiende `rollupOptions.input` para varios widgets:
...
En Twig, monta cada nodo por separado:
...
1. Ver código fuente — `<title>` y meta de `shareSeo()`.
2. Sin JS — título y precio unitario siguen visibles (opcional `noscript`).
3. Con JS — el widget muestra cantidad y total acumulado.

---

## Comparación de perfiles

| Perfil | SEO | Node en producción | Ejemplo |
|---------|-----|-------------------|---------|
| `twig` | Excelente | No | [Phonebook](./phonebook-app.md) |
| `hybrid` | Excelente | No | **Este tutorial** |
| `spa` | Baja prioridad | No | [Vue SPA](./vue-spa-app.md) |
