# Tutorial: aplicación de galería de imágenes

[← Volver al índice](../README.md)

Construye una **galería de imágenes**: formulario de subida, almacenamiento de archivos de Pinoox, cuadrícula de miniaturas y eliminación. Ideal para aprender **`File::upload()`** y vincular `file_id` a un modelo.

**Package:** `com_acme_gallery`  
**URL:** `http://localhost/pinoox/gallery`  
**Código fuente completo:** [docs/source/gallery-app/](../../source/gallery-app/) — copiar a `apps/`
---
...
## Paso 2 — Configuración de app.php

Habilita transport y filesystem para las subidas:
...
> **`File::upload('photo')`** lee el campo `<input type="file" name="photo">`. Los metadatos del archivo viven en la tabla de archivos de la plataforma; tu modelo solo almacena `file_id`.
...
No olvides **`enctype="multipart/form-data"`** en el formulario de subida.

---

## Paso 8 — Probar

1. Abre `http://localhost/pinoox/gallery`.
2. Sube un JPG/PNG — aparece en la cuadrícula con miniatura.
3. Haz clic en la imagen — tamaño completo en una nueva pestaña.
4. Elimina — la fila de la BD y el archivo físico se eliminan con `File::remove()`.

Si ves errores de permisos, comprueba el acceso de escritura en la carpeta `storage/` del proyecto.
...
| Mejora | Doc |
|---------|-----|
| Subidas múltiples | Bucle `File::upload()` o varios campos |
| Categorías de álbum | [Eloquent relationships](../eloquent-orm/relationships.md) |
...
- [Validation](../basic/validation.md) — reglas `image|max`
