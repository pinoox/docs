# Пошаговое руководство: доска задач (Todo)

[← Назад к оглавлению](../README.md)

**Доска ежедневных задач**: добавляйте задачи, отмечайте их выполненными и **фильтруйте** через строку запроса (`?status=pending`). Полезно для внутренних панелей и изучения фильтрации запросов в контроллерах.

**Пакет:** `com_acme_tasks`  
**URL:** `http://localhost/pinoox/tasks`  
**Полный исходный код:** [docs/source/task-board-app/](../../source/task-board-app/) — скопируйте в `apps/`
---

## Предварительные требования

- [Запросы (Requests)](../basic/requests.md) — параметры строки запроса
- [База данных / Eloquent](../database/getting-started.md)
...
> **Строка запроса (query string):** `$request->get('status')` читает `?status=pending` — дополнительный маршрут не нужен.

---

## Шаг 6 — Twig-шаблон (встроенный CSS)
...
2. Отметьте некоторые выполненными с помощью ✓.
3. Фильтр «Open» показывает только `pending`.
4. Используйте ↩, чтобы открыть задачу заново.
...
|---------|-----|
| Приоритет / срок выполнения | [Мутаторы и приведение типов (Mutators & casts)](../eloquent-orm/mutators-casts.md) |
| Мобильный API | [Руководство по Notes API](./simple-api-app.md) |
| Вход пользователей | [Управление пользователями](../advanced/user-management.md) |
...
- [Скоупы Eloquent](../eloquent-orm/getting-started.md)
