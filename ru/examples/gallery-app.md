# Пошаговое руководство: приложение галереи изображений

[← Назад к оглавлению](../README.md)

Создадим **галерею изображений**: форма загрузки, файловое хранилище Pinoox, сетка миниатюр и удаление. Отлично подходит для изучения **`File::upload()`** и связывания `file_id` с моделью.

**Пакет:** `com_acme_gallery`  
**URL:** `http://localhost/pinoox/gallery`  
**Полный исходный код:** [docs/source/gallery-app/](../../source/gallery-app/) — скопируйте в `apps/`
---

## Предварительные требования

- [Управление файлами](../advanced/file-management.md)
- [Миграции (Migrations)](../database/migrations.md)
...
## Шаг 2 — Настройки app.php

Включите transport и файловую систему для загрузок:
...
> **`File::upload('photo')`** читает поле `<input type="file" name="photo">`. Метаданные файла хранятся в платформенной таблице файлов; ваша модель хранит только `file_id`.

---

## Шаг 7 — Twig-шаблон (встроенный CSS)
...
Не забудьте **`enctype="multipart/form-data"`** в форме загрузки.

---

## Шаг 8 — Тестирование

1. Откройте `http://localhost/pinoox/gallery`.
2. Загрузите JPG/PNG — изображение появится в сетке с миниатюрой.
3. Кликните по изображению — полный размер в новой вкладке.
4. Удалите — строка в БД и физический файл удаляются через `File::remove()`.

Если вы видите ошибки прав доступа, проверьте права на запись в папке `storage/` проекта.
...
|---------|-----|
| Множественная загрузка | Цикл с `File::upload()` или несколько полей |
| Категории альбомов | [Связи в Eloquent](../eloquent-orm/relationships.md) |
| Мобильный API | [Руководство по Notes API](./simple-api-app.md) |
| S3 / CDN | [Управление файлами](../advanced/file-management.md) |

---

## Связанная документация

- [Валидация (Validation)](../basic/validation.md) — правила `image|max`
