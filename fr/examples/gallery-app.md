# Tutoriel pas à pas : Application de galerie d'images

[← Retour à l'index](../README.md)

Construisez une **galerie d'images** : formulaire d'upload, stockage de fichiers Pinoox, grille de miniatures et suppression. Idéal pour apprendre **`File::upload()`** et lier un `file_id` à un modèle.

**Package :** `com_acme_gallery`  
**URL :** `http://localhost/pinoox/gallery`  
**Source complète :** [docs/source/gallery-app/](../../source/gallery-app/) — à copier dans `apps/`
---

## Prérequis

- [Gestion de fichiers](../advanced/file-management.md)
...
## Étape 2 — Paramètres app.php

Activez transport et filesystem pour les uploads :
...
> **`File::upload('photo')`** lit le champ `<input type="file" name="photo">`. Les métadonnées du fichier sont stockées dans la table de fichiers de la plateforme ; votre modèle ne stocke que le `file_id`.
...
N'oubliez pas **`enctype="multipart/form-data"`** sur le formulaire d'upload.

---

## Étape 8 — Tester

1. Ouvrez `http://localhost/pinoox/gallery`.
2. Uploadez un JPG/PNG — il apparaît dans la grille avec une miniature.
3. Cliquez sur l'image — taille réelle dans un nouvel onglet.
4. Supprimez — la ligne en base de données et le fichier physique sont supprimés via `File::remove()`.

Si vous voyez des erreurs de permissions, vérifiez les droits d'écriture sur le dossier `storage/` du projet.
...
| Amélioration | Doc |
|---------|-----|
| Uploads multiples | Boucler sur `File::upload()` ou utiliser plusieurs champs |
| Catégories d'albums | [Relations Eloquent](../eloquent-orm/relationships.md) |
| API mobile | [Tutoriel API de notes](./simple-api-app.md) |
| S3 / CDN | [Gestion de fichiers](../advanced/file-management.md) |

---

## Documentation associée

- [Validation](../basic/validation.md) — règles `image|max`
