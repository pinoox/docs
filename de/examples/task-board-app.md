...
Ein **tägliches Task-Board**: Aufgaben hinzufügen, als erledigt markieren und **filtern** mit einem Query-String (`?status=pending`). Praktisch für interne Panels und zum Lernen von Query-Filterung in Controllern.

**Package:** `com_acme_tasks`  
**URL:** `http://localhost/pinoox/tasks`  
**Vollständiger Quellcode:** [docs/source/task-board-app/](../../source/task-board-app/) — nach `apps/` kopieren
---

## Voraussetzungen

- [Requests](../basic/requests.md) — Query-Parameter
- [Datenbank / Eloquent](../database/getting-started.md)
...
> **Query-String:** `$request->get('status')` liest `?status=pending` — keine zusätzliche Route nötig.

---

## Schritt 6 — Twig-Template (Inline-CSS)
...
1. Mehrere Aufgaben hinzufügen.
2. Einige mit ✓ als erledigt markieren.
3. Filter „Open“ zeigt nur `pending`.
4. Mit ↩ wieder öffnen.
...
|---------|-----|
| Priorität / Fälligkeitsdatum | [Mutatoren & Casts](../eloquent-orm/mutators-casts.md) |
| Mobile API | [Notes-API-Walkthrough](./simple-api-app.md) |
| Benutzer-Login | [Benutzerverwaltung](../advanced/user-management.md) |
