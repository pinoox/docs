# Walkthrough: Bildergalerie-App

[← Zurück zur Übersicht](../README.md)

Erstellen Sie eine **Bildergalerie**: Upload-Formular, Pinoox-Dateispeicher, Thumbnail-Raster und Löschfunktion. Gut geeignet, um **`File::upload()`** und die Verknüpfung von `file_id` mit einem Model kennenzulernen.

**Package:** `com_acme_gallery`  
**URL:** `http://localhost/pinoox/gallery`  
**Vollständiger Quellcode:** [docs/source/gallery-app/](../../source/gallery-app/) — nach `apps/` kopieren
---
...
## Schritt 2 — Einstellungen in app.php

Transport und Filesystem für Uploads aktivieren:
...
## Schritt 3 — Tabelle `gallery_items`
...
> **`File::upload('photo')`** liest das Feld `<input type="file" name="photo">`. Die Dateimetadaten liegen in der Plattform-Dateitabelle; Ihr Model speichert nur die `file_id`.
...
Vergessen Sie nicht **`enctype="multipart/form-data"`** im Upload-Formular.

---

## Schritt 8 — Testen

1. Öffnen Sie `http://localhost/pinoox/gallery`.
2. Laden Sie ein JPG/PNG hoch — es erscheint im Raster mit einem Thumbnail.
3. Klicken Sie auf das Bild — Vollbild in einem neuen Tab.
4. Löschen — Datenbankzeile und physische Datei werden über `File::remove()` entfernt.

Wenn Berechtigungsfehler auftreten, prüfen Sie die Schreibrechte auf dem Projektordner `storage/`.
...
|---------|-----|
| Mehrfach-Uploads | `File::upload()` in einer Schleife oder mehrere Felder |
| Album-Kategorien | [Eloquent-Beziehungen (Relationships)](../eloquent-orm/relationships.md) |
| Mobile API | [Notes-API-Walkthrough](./simple-api-app.md) |
| S3 / CDN | [Dateiverwaltung (File management)](../advanced/file-management.md) |

---

## Verwandte Dokumente

- [Validierung (Validation)](../basic/validation.md) — Regeln `image|max`
