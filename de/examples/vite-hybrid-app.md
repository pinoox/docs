...
Eine **Produktseite** mit vollständigem HTML in Twig (SEO und Meta in PHP) plus ein interaktives **Vite-Widget** (Vanilla JS), das den Preis nach Menge berechnet. Profil **hybrid** — ohne Vue/React.

**Package:** `com_acme_vite_shop`  
**URL:** `http://localhost/pinoox/shop`  
**Profile:** `hybrid` · **stack:** `vite`  
**Vollständiger Quellcode:** [docs/source/vite-hybrid-app/](../../source/vite-hybrid-app/) — nach `apps/` kopieren
---

## Voraussetzungen

- Node.js 18+
- [Views](../basic/views.md) und [`vite_tags()`](../basic/templates.md)
...
## Schritt 3 — Vite im Theme (ohne Vue/React)
...
## Schritt 4 — Vanilla-JS-Widget

**`src/boot.js`** (gemeinsam mit SPA-Stubs):
...
## Schritt 5 — Twig-Template (Outline)
...
> **Hybrid-Regel:** Titel, Beschreibung und Hauptinhalt liegen in Twig — das Widget mountet nur den interaktiven Block. Anfangsdaten kommen aus `bootstrap`, nicht aus einem doppelten API-Fetch.

---

## Schritt 6 — `.env` und Dev
...
Seite in MAMP öffnen; Mengenänderung aktualisiert die Live-Summe.
...
## Mehrere Einstiegspunkte (optional)

Erweitern Sie `rollupOptions.input` für mehrere Widgets:
...
In Twig jeden Knoten separat mounten:
...
1. Seitenquelltext anzeigen — `<title>` und Meta aus `shareSeo()`.
2. Ohne JS — Titel und Stückpreis weiterhin sichtbar (optional `noscript`).
3. Mit JS — Widget zeigt Menge und laufende Summe.

---

## Profilvergleich

| Profil | SEO | Node in Produktion | Beispiel |
|---------|-----|-------------------|---------|
| `twig` | Ausgezeichnet | Nein | [Phonebook](./phonebook-app.md) |
| `hybrid` | Ausgezeichnet | Nein | **Dieser Walkthrough** |
| `spa` | Niedrige Priorität | Nein | [Vue SPA](./vue-spa-app.md) |
