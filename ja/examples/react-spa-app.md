# ウォークスルー: React SPA パネル

[← 索引に戻る](../README.md)

**React 18 タスクパネル**（Todo）— API は PHP、UI は Vite 付き React。Pinoox のデフォルト React エントリ: `src/main.jsx`。

**Package:** `com_acme_react_tasks`  
**URL:** `http://localhost/pinoox/react-tasks`  
**Profile:** `spa` · **stack:** `react`  
**完全なソース:** [docs/source/react-spa-app/](../../source/react-spa-app/) — `apps/` にコピー
---

## 前提条件

- Node.js 18+ と npm
- JSON API 向け [Request](../basic/requests.md) と [Response](../basic/responses.md)
...
| トピック | React | Vue |
|-------|-------|-----|
| エントリ | `src/main.jsx` | `src/main.js` |
| スキャフォールド | `--stack=react` | `--stack=vue` |
| ブートストラップ | `boot.js` 内の `getUrl()` | 同じ |

---

## 関連ドキュメント

- [Vue SPA ウォークスルー](./vue-spa-app.md)
