...
**画像ギャラリー** を構築: アップロードフォーム、Pinoox ファイル storage、サムネイルグリッド、削除。**`File::upload()`** の学習と Model への `file_id` リンクに適しています。

**Package:** `com_acme_gallery`  
**URL:** `http://localhost/pinoox/gallery`  
**完全なソース:** [docs/source/gallery-app/](../../source/gallery-app/) — `apps/` にコピー
---
...
## ステップ 2 — app.php settings

アップロード用に transport と filesystem を有効化:
...
> **`File::upload('photo')`** reads the `<input type="file" name="photo">` field. File metadata lives in the platform file table; your model stores only `file_id`.
...
アップロードフォームに **`enctype="multipart/form-data"`** を忘れないでください。

---

## ステップ 8 — テスト

1. `http://localhost/pinoox/gallery` を開く。
2. JPG/PNG をアップロード — サムネイル付きでグリッドに表示される。
3. 画像をクリック — 新しいタブでフルサイズ表示。
4. 削除 — `File::remove()` により DB 行と物理ファイルが削除される。

権限エラーが出る場合、プロジェクト `storage/` フォルダの書き込み権限を確認してください。
...
| 拡張 | ドキュメント |
|---------|-----|
| 複数アップロード | `File::upload()` をループするか複数フィールドを使用 |
| アルバムカテゴリ | [Eloquent relationships](../eloquent-orm/relationships.md) |
