# ウォークスルー: シンプルなブログ アプリ

[← 索引に戻る](../README.md)

小さなブログを構築: **投稿一覧**、URL に slug がある **個別投稿ページ**、**投稿作成** フォーム。動的ルートと複数 Twig ページの学習に適しています。

**Package:** `com_acme_blog`  
**URL:** `http://localhost/pinoox/blog`  
**完全なソース:** [docs/source/blog-app/](../../source/blog-app/) — `apps/` にコピー
---

## 前提条件

- [Router](../basic/routers.md) — `{slug}` パラメータ
...
> **動的ルート:** `{slug}` は Controller に `$slug` として渡されます。
...
| 拡張 | ドキュメント |
|---------|-----|
| ページネーション一覧 | [Pagination](../database/pagination.md) |
| `/write` を保護 | [Flows / Auth](../basic/flows.md) |
