...
**日次タスクボード**: タスクを追加し、完了にマークし、クエリ文字列（`?status=pending`）で **フィルタ** します。内部パネルや Controller でのクエリフィルタリングの学習に実用的です。

**Package:** `com_acme_tasks`  
**URL:** `http://localhost/pinoox/tasks`  
**完全なソース:** [docs/source/task-board-app/](../../source/task-board-app/) — `apps/` にコピー
---

## 前提条件

- [Request](../basic/requests.md) — クエリパラメータ
...
> **Query string:** `$request->get('status')` reads `?status=pending` — no extra route needed.
...
| 拡張 | ドキュメント |
|---------|-----|
| 優先度 / 期限 | [Mutators & casts](../eloquent-orm/mutators-casts.md) |
| モバイル API | [Notes API ウォークスルー](./simple-api-app.md) |
| ユーザーログイン | [User management](../advanced/user-management.md) |
