...
Protocol id: `pinion` آ· version: `2`
...
Three HTTP steps: **init â†’ upload parts â†’ complete**.
...
| `local` | Assemble to project path via `path()` â€” no `pinx_file` row |
...
**Local-only** (no Flysystem â€” e.g. manager `.pinx` packages):
...
| Abort | `POST {prefix}/abort/{id}` | â€” |

### Managed complete response

```json
{
  "session": { "id": "â€¦", "filename": "video.mp4" },
  "path": "uploads/media/x7f2.mp4",
  "file_id": 42,
  "url": "https://cdn.example.com/â€¦",
  "storage": true
}
```
...
Single-app projects â€” same commands via [Pinx CLI](../start/pinx-cli.md):
...
Frontend: `theme/spark/src/utils/pinion.js` â€” threshold 8 MB, parallel parts, SHA-256 hashes.
...
sequenceDiagram
    ...
    Pinion->>Storage: assemble â†’ Flysystem / local path
