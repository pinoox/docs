...
| `patch:create` / `patch:run` / `patch:status` / `patch:rollback` | [Patch'ler](../advanced/patches.md) |
| `query` | Ham SQL (debug) |
...
Devam eden parçalı yükleme oturumlarını yönetin (geçici depolama: `storage/pinion`):
