# Uygulamalı rehber: Resim galerisi uygulaması

[← Dizine dön](../README.md)

**Resim galerisi** oluşturun: yükleme formu, Pinoox dosya depolama, küçük resim ızgarası ve silme. **`File::upload()`** ve `file_id`'yi bir model'e bağlamayı öğrenmek için uygundur.

**Paket:** `com_acme_gallery`  
**URL:** `http://localhost/pinoox/gallery`  
**Tam kaynak:** [docs/source/gallery-app/](../../source/gallery-app/) — `apps/` içine kopyalayın
---
...
## Adım 2 — app.php ayarları

Yüklemeler için transport ve filesystem etkinleştirin:
...
> **`File::upload('photo')`** `<input type="file" name="photo">` alanını okur. Dosya meta verileri platform dosya tablosunda tutulur; modeliniz yalnızca `file_id` saklar.

---

## Adım 7 — Twig şablonu (inline CSS)
...
Yükleme formunda **`enctype="multipart/form-data"`** eklemeyi unutmayın.

---

## Adım 8 — Test

1. `http://localhost/pinoox/gallery` adresini açın.
2. JPG/PNG yükleyin — ızgarada küçük resimle görünür.
3. Resme tıklayın — yeni sekmede tam boyut.
4. Silin — DB satırı ve fiziksel dosya `File::remove()` ile kaldırılır.

İzin hataları görürseniz proje `storage/` klasöründe yazma erişimini kontrol edin.
...
|---------|-----|
| Çoklu yükleme | `File::upload()` döngüsü veya birden fazla alan |
| Albüm kategorileri | [Eloquent ilişkileri](../eloquent-orm/relationships.md) |
| Mobil API | [Notes API uygulamalı rehber](./simple-api-app.md) |
| S3 / CDN | [File management](../advanced/file-management.md) |

---

## İlgili dokümantasyon

- [Validation](../basic/validation.md) — `image|max` rules
