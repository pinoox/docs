# Uygulamalı rehber: Vue SPA paneli

[← Dizine dön](../README.md)

Pinoox API'den notları okuyan ve basit CRUD yapan **Vue 3 tek sayfalı panel** oluşturun. Desen `com_pinoox_manager` / `spark` ile eşleşir: Twig yalnızca **kabuk**; UI Vue bileşenlerinde yaşar.

**Paket:** `com_acme_vue_notes`  
**URL:** `http://localhost/pinoox/vue-notes`  
**Profil:** `spa` · **stack:** `vue`  
**Tam kaynak:** [docs/source/vue-spa-app/](../../source/vue-spa-app/) — `apps/` içine kopyalayın
---

## Ön koşullar

- PHP 8.2+ ile Pinoox 3.x
- Node.js 18+ ve npm
- [Notes API uygulamalı rehber](./simple-api-app.md) ve [Twig şablonları](../basic/templates.md) bilgisi

---

## Adım 1 — Uygulamayı ve route'u oluşturun

```bash
php pinoox app:create com_acme_vue_notes --simple
php pinoox app:router set /vue-notes com_acme_vue_notes
```

---

## Adım 2 — Notes API (sunucu tarafı)

Tablo, Model, `NoteApiController` ve `routes/api.php` dosyalarını [simple-api-app](./simple-api-app.md) rehberindeki gibi oluşturun (aynı `title` ve `body` ile `notes` migration'ı).

API route dosyasını `app.php` içinde kaydedin:

```php
'router' => [
    'routes' => [
        'routes/web.php',
        'routes/actions.php',
        'routes/api.php',
    ],
],
```

---

## Adım 3 — Vue frontend iskeleti

```bash
php pinoox fe com_acme_vue_notes scaffold --stack=vue
php pinoox fe com_acme_vue_notes install
```

Tema dosyaları `theme/default/` içine düşer: `main.twig`, `vite.config.js`, `src/main.js`, `src/boot.js` vb.

---

## Adım 4 — `frontend.config.php`

```php
<?php

return [
    'profile' => 'spa',
    'stack' => 'vue',
    'entry' => 'src/main.js',
    'manifest' => 'dist/.vite/manifest.json',
    'mount' => '#app',
    'dev' => [
        'enabled' => (bool) _env('VITE_DEV', false),
        'url' => rtrim((string) _env('VITE_DEV_SERVER', 'http://127.0.0.1:5173'), '/'),
    ],
];
```

Bu değerleri `app.php` içinde yansıtabilirsiniz:

```php
'frontend' => [
    'profile' => 'spa',
    'stack' => 'vue',
],
```

---

## Adım 5 — SPA route'u (catch-all)

`routes/web.php`:

```php
<?php

use Pinoox\Portal\View;
use function Pinoox\Router\{get, routes};

return routes(function () {
    get('*', fn () => View::render('main'))->name('fallback');
});
```

`/api/v1/…` dışındaki her uygulama URL'si `main.twig` sunar; istemci tarafı yolları Vue Router yönetir.

---

## Adım 6 — Twig kabuğu

`theme/default/main.twig` (özet):

```twig
<!doctype html>
<html lang="en" dir="ltr">
<head>
    {% include 'partials/head.twig' %}
    {% include 'partials/scripts.twig' %}
</head>
<body>
    <div id="app"></div>
</body>
</html>
```

`theme/default/partials/scripts.twig`:

```twig
{{ pinoox_bootstrap(bootstrap|default({}))|raw }}
{{ vite('src/main.js') }}
```

`pinoox_bootstrap()`, axios/fetch çağrıları için `url.API` dahil `window.__PINOOX__` enjekte eder.

---

## Adım 7 — Tema `.env` (geliştirme)

`theme/default/.env` (copy from `.env.example`):

```env
VITE_SERVER_URL=http://localhost/pinoox/vue-notes
VITE_DEV=true
VITE_DEV_SERVER=http://127.0.0.1:5173
VITE_API_PATH=/api/v1/
VITE_LOCALE=en
```

MAMP alt klasör kurulumlarında `VITE_SERVER_URL` **tam PHP origin** olmalıdır (yol öneki dahil).

---

## Adım 8 — Vue istemcisi (minimal)

`src/api/notes.js`:

```js
import axios from 'axios';
import { getUrl } from '../boot.js';

const api = axios.create({
    baseURL: getUrl().API,
    headers: { Accept: 'application/json' },
});

export async function fetchNotes() {
    const { data } = await api.get('/notes');
    return data.data ?? [];
}

export async function createNote(payload) {
    const { data } = await api.post('/notes', payload);
    return data.data;
}

export async function deleteNote(id) {
    await api.delete(`/notes/${id}`);
}
```

`src/App.vue` (CSS in the same file):

```vue
<script setup>
import { onMounted, ref } from 'vue';
import { createNote, deleteNote, fetchNotes } from './api/notes.js';

const notes = ref([]);
const title = ref('');
const body = ref('');
const loading = ref(true);

async function load() {
    loading.value = true;
    notes.value = await fetchNotes();
    loading.value = false;
}

async function add() {
    if (!title.value.trim()) return;
    await createNote({ title: title.value, body: body.value });
    title.value = '';
    body.value = '';
    await load();
}

async function remove(id) {
    await deleteNote(id);
    await load();
}

onMounted(load);
</script>

<template>
    <main class="page">
        <h1 class="page-title">Notes (Vue SPA)</h1>
        <div class="panel">
            <form class="form" @submit.prevent="add">
                <div class="field">
                    <label>Title</label>
                    <input v-model="title" required />
                </div>
                <div class="field">
                    <label>Body</label>
                    <textarea v-model="body" rows="3"></textarea>
                </div>
                <button type="submit" class="btn btn-primary">Save</button>
            </form>
        </div>
        <div class="panel">
            <p v-if="loading" class="empty">Loading…</p>
            <ul v-else class="note-list">
                <li v-for="n in notes" :key="n.id">
                    <div>
                        <strong>{{ n.title }}</strong>
                        <p v-if="n.body" class="note-body">{{ n.body }}</p>
                    </div>
                    <button type="button" class="btn btn-danger btn-sm" @click="remove(n.id)">Delete</button>
                </li>
            </ul>
            <p v-if="!loading && !notes.length" class="empty">No notes yet.</p>
        </div>
    </main>
</template>

<style scoped>
*, *::before, *::after { box-sizing: border-box; }
.page { max-width: 560px; margin: 0 auto; padding: 2rem 1rem; font-family: system-ui, sans-serif; line-height: 1.5; color: #0f172a; }
.page-title { margin: 0 0 1.25rem; padding-bottom: .75rem; border-bottom: 2px solid #334155; font-size: 1.5rem; }
.panel { background: #fff; border: 2px solid #cbd5e1; border-radius: 10px; padding: 1.25rem 1.5rem; margin-bottom: 1rem; }
.field { margin-bottom: 1rem; }
.field label { display: block; font-weight: 600; margin-bottom: .35rem; font-size: .9rem; }
.field input, .field textarea { width: 100%; padding: .5rem .65rem; border: 2px solid #cbd5e1; border-radius: 6px; font: inherit; }
.btn { padding: .45rem 1rem; font: inherit; font-weight: 600; border-radius: 6px; cursor: pointer; background: transparent; }
.btn-primary { border: 2px solid #2563eb; color: #2563eb; }
.btn-primary:hover { background: #2563eb; color: #fff; }
.btn-danger { border: 2px solid #dc2626; color: #dc2626; }
.btn-danger:hover { background: #dc2626; color: #fff; }
.btn-sm { padding: .25rem .6rem; font-size: .85rem; }
.note-list { list-style: none; padding: 0; margin: 0; }
.note-list li { display: flex; align-items: flex-start; justify-content: space-between; gap: .75rem; padding: .75rem 0; border-bottom: 1px solid #e2e8f0; }
.note-body { margin: .25rem 0 0; color: #475569; font-size: .95rem; }
.empty { color: #64748b; font-style: italic; margin: 0; }
</style>
```

İskelet genellikle Pinia ve Vue Router ile gelir; bu rehberde Router'ı kaldırıp `App.vue`'yu doğrudan `src/main.js`'den mount edebilirsiniz.

---

## Adım 9 — Geliştirme (HMR)

```bash
php pinoox fe com_acme_vue_notes dev --no-serve
```

Apache/MAMP zaten PHP sunuyorsa `--no-serve` kullanın. PHP ve Vite'i birlikte çalıştırmak için:

```bash
php pinoox fe com_acme_vue_notes dev
```

`http://localhost/pinoox/vue-notes` adresini açın — Vue hot-reload yapar; Vite dev sunucusu `/api`'yi PHP'ye proxy'ler.

---

## Adım 10 — Üretim derlemesi

```bash
php pinoox fe com_acme_vue_notes build
# or: php pinoox theme:frontend build com_acme_vue_notes
```

Çıktı `theme/default/dist/` içine gider — Twig üretim `<script>` / `<link>` etiketleri için manifest'i okur.

---

## Test

1. Migration'ları çalıştırın ve API'yi curl ile doğrulayın (bkz. [simple-api-app](./simple-api-app.md)).
2. SPA'yı açın — boş veya dolu liste render edilmelidir.
3. Tam sayfa yenilemesi olmadan not ekleyin ve silin.

---

## Sonraki adımlar

| Geliştirme | Dokümantasyon |
|---------|-----|
| Vue Router + birden fazla sayfa | Varsayılan iskelet `src/router/` |
| Auth ve Flow middleware | [Flow'lar](../basic/flows.md) |
| Hibrit profil (SEO + Vue adası) | [Vite hibrit uygulamalı rehber](./vite-hybrid-app.md) |

---

## İlgili dokümantasyon

- [Templates & `vite_tags()`](../basic/templates.md)
- [CLI — `theme:frontend`](../start/cli-reference.md)
- [app.php — `frontend` key](../start/app-manifest.md)

---

[← Dizine dön](../README.md)
