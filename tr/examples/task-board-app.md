# Uygulamalı rehber: Görev panosu (Todo)

[← Dizine dön](../README.md)

**Günlük görev panosu**: görev ekleyin, tamamlandı işaretleyin ve query string ile **filtreleyin** (`?status=pending`). Dahili paneller ve controller'larda sorgu filtrelemesi öğrenmek için pratiktir.

**Paket:** `com_acme_tasks`  
**URL:** `http://localhost/pinoox/tasks`  
**Tam kaynak:** [docs/source/task-board-app/](../../source/task-board-app/) — `apps/` içine kopyalayın
---
...
> **Sorgu dizesi:** `$request->get('status')` ifadesi `?status=pending` değerini okur — ek route gerekmez.

---

## Adım 6 — Twig şablonu (inline CSS)
...
2. Bazılarını ✓ ile tamamlandı işaretleyin.
3. "Açık" filtresi yalnızca `pending` gösterir.
4. Yeniden açmak için ↩ kullanın.
...
|---------|-----|
| Öncelik / bitiş tarihi | [Mutator'lar ve cast'ler](../eloquent-orm/mutators-casts.md) |
| Mobil API | [Notes API uygulamalı rehber](./simple-api-app.md) |
| Kullanıcı girişi | [Kullanıcı yönetimi](../advanced/user-management.md) |
