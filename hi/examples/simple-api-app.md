...
यह guide notes (title + body) के लिए **JSON API** expose करने वाली छोटी app बनाती है। मान लें Pinoox install है और project root से `php pinoox` काम करता है।
...
Wizard में display name `Notes` और path `/notes` set करें (या बाद में CLI से path register करें)।

---

## Step 2 — App URL register करें
...
JSON responses Pinoox standard envelope उपयोग करते हैं: `{ success, data, message, meta }`.
...
## Step 7 — `app.php` में `api.php` register करें
...
Project `http://localhost/pinoox` पर हो:
...
> **Named Action क्या है?** API routes में आमतौर पर सीधे `[Controller::class, 'method']` point करते हैं। Named Actions (`@home`) HTML pages के लिए `web.php` में ज़्यादा उपयोगी — [Routers](../basic/routers.md) देखें।

---

## Step 9 — (Optional) CSS के साथ HTML page

Browser में notes देखने के लिए inline CSS के साथ simple Twig page add करें:
...
URL: `http://localhost/pinoox/notes/browse` — API `/api/v1/notes` पर रहता है।
