...
Twig में full HTML (PHP में SEO और meta) के साथ **product page**, plus quantity से price calculate करने वाला interactive **Vite widget** (Vanilla JS)। **Hybrid** profile — Vue/React नहीं।
...
## Step 3 — Theme में Vite (no Vue/React)
...
> **Hybrid rule:** title, description, और main content Twig में रहते हैं — widget केवल interactive block mount करता है। Initial data `bootstrap` से आता है, duplicate API fetch नहीं।
...
MAMP में page खोलें; quantity बदलने पर live total update होता है।
...
Manifest `dist/.vite/manifest.json` पर — production में Twig सही `<script>` tag inject करता है।

---

## Multiple entries (optional)

कई widgets के लिए `rollupOptions.input` extend करें:
...
Twig में हर node separately mount करें:
...
1. View Source — `<title>` और meta `shareSeo()` से।
2. JS के बिना — title और unit price अभी भी visible (optional `noscript`)।
3. JS के साथ — widget quantity और running total दिखाता है।
