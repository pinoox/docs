...
构建一个小型博客：**文章列表**、URL 中带 slug 的**单篇文章页**，以及一个**写文章**表单。非常适合学习动态路由和多个 Twig 页面。

**包名（Package）：** `com_acme_blog`  
**URL：** `http://localhost/pinoox/blog`  
**完整源码：** [docs/source/blog-app/](../../source/blog-app/) — 复制到 `apps/`
---

## 前置条件

- [路由（Routers）](../basic/routers.md) — `{slug}` 参数
...
1. `/blog/write` — 创建一篇文章。
2. 提交后你会跳转到 `/blog/post/your-slug`。
3. `/blog` 显示文章列表。
...
| 升级方向 | 文档 |
|---------|-----|
| 分页列表 | [分页（Pagination）](../database/pagination.md) |
| 保护 `/write` | [Flows / Auth](../basic/flows.md) |
