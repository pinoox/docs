...
一个**日常任务看板**：添加任务、标记完成，并通过查询字符串（`?status=pending`）进行**筛选**。非常适合内部管理面板，也适合学习在控制器中进行查询过滤。

**包名（Package）：** `com_acme_tasks`  
**URL：** `http://localhost/pinoox/tasks`  
**完整源码：** [docs/source/task-board-app/](../../source/task-board-app/) — 复制到 `apps/`
---

## 前置条件

- [请求（Requests）](../basic/requests.md) — 查询参数
- [数据库 / Eloquent](../database/getting-started.md)
...
> **查询字符串（Query string）：** `$request->get('status')` 读取 `?status=pending` — 无需额外的路由。
...
| 升级方向 | 文档 |
|---------|-----|
| 优先级 / 截止日期 | [修改器与类型转换（Mutators & casts）](../eloquent-orm/mutators-casts.md) |
| 移动端 API | [笔记 API 实战演练](./simple-api-app.md) |
| 用户登录 | [用户管理](../advanced/user-management.md) |
...
- [Eloquent 作用域（scopes）](../eloquent-orm/getting-started.md)
