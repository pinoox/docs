...
一个 **React 18 任务面板**（Todo）— API 用 PHP 编写，UI 使用 React 与 Vite。Pinoox 默认的 React 入口：`src/main.jsx`。

**包名（Package）：** `com_acme_react_tasks`  
**URL：** `http://localhost/pinoox/react-tasks`  
**Profile：** `spa` · **stack：** `react`  
**完整源码：** [docs/source/react-spa-app/](../../source/react-spa-app/) — 复制到 `apps/`
---

## 前置条件

- Node.js 18+ 和 npm
- 用于 JSON API 的[请求（Requests）](../basic/requests.md)与[响应（Responses）](../basic/responses.md)
...
| 主题 | React | Vue |
|-------|-------|-----|
| 入口 | `src/main.jsx` | `src/main.js` |
| 脚手架 | `--stack=react` | `--stack=vue` |
| 引导（Bootstrap） | `boot.js` 中的 `getUrl()` | 相同 |

---

## 相关文档

- [Vue SPA 实战演练](./vue-spa-app.md)
- [任务看板（Twig）](./task-board-app.md)
- [模板（Templates）](../basic/templates.md)
