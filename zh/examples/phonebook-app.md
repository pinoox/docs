...
使用 Twig 构建一个简单的 **Web 应用**：列出联系人、添加表单、删除条目。数据存储在 MySQL 中。

**包名（Package）：** `com_acme_phonebook`  
**URL：** `http://localhost/pinoox/phonebook`  
**完整源码：** [docs/source/phonebook-app/](../../source/phonebook-app/) — 复制到 `apps/`
---

## 前置条件

- 已安装 Pinoox
- 基础的[路由（Routers）](../basic/routers.md)和[视图（Views）](../basic/views.md)知识
...
> **为什么要有两个路由文件？** 在 `actions.php` 中你只需定义一次处理器（handler）。在 `web.php` 中你只把 URL + HTTP 方法映射到该 Action（`@contact.list`）。这样修改 URL 时无需改动控制器的接线。
...
文件 `theme/default/pages/list.twig`：
...
| 升级方向 | 相关文档 |
|---------|-------------|
| 编辑联系人 | [控制器（Controllers）](../basic/controllers.md) |
| 搜索 | [查询构造器（Query Builder）](../database/query-builder.md) |
| 登录 | [Flows / Auth](../basic/flows.md) |
