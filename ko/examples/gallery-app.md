# 워크스루: Image gallery 앱

[← 색인으로 돌아가기](../README.md)

**image gallery** 구축: upload form, Pinoox file storage, thumbnail grid, delete. **`File::upload()`** 학습과 model에 `file_id` 연결에 좋음.

**Package:** `com_acme_gallery`  
**URL:** `http://localhost/pinoox/gallery`  
**Full source:** [docs/source/gallery-app/](../../source/gallery-app/) — `apps/`에 copy
---
...
## 단계 2 — app.php settings

storage용 transport 및 파일 시스템 활성화:
...
> **`File::upload('photo')`**는 `<input type="file" name="photo">` field를 읽습니다. File metadata는 platform file table에; model은 `file_id`만 저장.
...
업로드 양식에 **`enctype="multipart/form-data"`** 를 빠뜨리지 마세요.

---

## 단계 8 — Test

1. Open `http://localhost/pinoox/gallery`.
2. Upload a JPG/PNG — it appears in the grid with a thumbnail.
3. Click the image — full size in a new tab.
4. Delete — DB row and physical file are removed via `File::remove()`.

permission 오류가 보이면 프로젝트 `storage/` 폴더 쓰기 access를 확인하세요.
...
| Upgrade | Doc |
|---------|-----|
| Multiple uploads | Loop `File::upload()` or multiple fields |
| Album categories | [Eloquent relationships](../eloquent-orm/relationships.md) |
