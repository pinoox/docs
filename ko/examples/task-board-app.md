# 워크스루: Task board (Todo)

[← 색인으로 돌아가기](../README.md)

**daily task board**: task 추가, 완료 표시, query string(`?status=pending`)으로 **filter**. 내부 panel과 Controller에서 query filtering 학습에 실용적.

**Package:** `com_acme_tasks`  
**URL:** `http://localhost/pinoox/tasks`  
**Full source:** [docs/source/task-board-app/](../../source/task-board-app/) — `apps/`에 copy
---
...
> **Query string:** `$request->get('status')`가 `?status=pending`을 읽음 — 추가 route 불필요.

---

## 단계 6 — Twig template (inline CSS)
...
1. Add several tasks.
2. Mark some done with ✓.
3. Filter “Open” shows only `pending`.
4. Use ↩ to reopen.
